...
update_variables_server_2 <- function(id,
                                    data,
                                    height = NULL,
    ...
    module = function(input, output, session) {
      ...
      observeEvent(input$validate, {
        ...
        if (inherits(res_update, "try-error")) {
          ...
          insert_alert(
            selector = ns("update"),
            title = tags$b(phosphoricons::ph("check"), i18n("Data successfully updated!")),
            type = "success"
          )
          updated_data$x <- data
...
update_variables_datagrid <- function(data, height = NULL, selectionId = NULL, buttonId = NULL) {
  ...
  grid <- toastui::grid_columns(
    grid = grid,
    columns = c("name", "class", "class_toset", "n_missing", "p_complete", "n_unique"),
    header = c("Name", "Class", "New class", "Missing values", "Complete obs.", "Unique values"),
    minWidth = 120
  )
...
insert_error <- function(
  mssg = i18n("Something went wrong..."),
  session = shiny::getDefaultReactiveDomain()
) {
  show_alert(
    title = tags$b(phosphoricons::ph("warning"), i18n("Ooops")),
    text = mssg,
    type = "error"
  )
}

insert_alert <- function(selector, title = NULL, text = NULL, type = "info") {
  removeUI(selector = paste0("#", selector, "-result"), immediate = TRUE)
  ...
  insertUI(
    ...
    ui = tags$div(
      ...
      style = paste0(
        "border:1px solid ", color, "; padding:15px; border-radius:5px; ",
